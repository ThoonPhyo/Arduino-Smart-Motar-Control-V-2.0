...
int water_flow_check_delay = 60000; // water flow check delay 1 minute after motar started
...
// ===== Full Tank Stable Stop Delay =====
unsigned long fullDetectTime = 0;
bool fullDelayActive = false;
const unsigned long FULL_DELAY = 120000; // 2 minutes
...
// ---------- Full Tank Stable Stop Delay ---------
bool fullTankDelayHandler() {

  if (full && half && motorState && !water_flow_issue) {

    if (!fullDelayActive) {
      fullDelayActive = true;
      fullDetectTime = millis();
    }

    if (millis() - fullDetectTime >= FULL_DELAY) {
      fullDelayActive = false;
      return true;   // Tell system: OK stop motor now
    }
  }

  // If water drops before delay time → cancel timer
  if (!full) {
    fullDelayActive = false;
  }

  return false;  // Do not stop motor yet
}
      ...
      if (previous_mode == 2 && motorState) {
        ...
        if (voltage < save_voltage) {
          ...
          if (motorState) {
            ...
            Serial.println("Motor stopped due to low voltage ");
          }
        }
        ...
        // Stop if tank full
        if (fullTankDelayHandler()) {
            digitalWrite(MOTOR_RELAY, LOW);
            motorState = false;
            Motar_run_time = 0;
            water_flow_issue = false;
            Serial.println("Motor stopped after stable full delay");
        }
      }
